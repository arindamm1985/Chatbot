...
def generate_keywords(title: str, description: str, content: str):
    ...
    Please analyze the following website details and extract the primary business type (e.g., Digital Marketing and Branding Agency, Legal Operations Support, Business Directory Service, etc.) that best describes the business.
    Return only the business type as a short phrase. Do not include any labels or markers such as "Part 1:" in your output.
    ...
    7. Return ONLY the final list of SEO keywords, separated by commas, with no extra text or explanation. The final list should contain at least 10 and at most 15 keywords. The extracted business type should appear only once at the beginning. Do NOT repeat the business type in the keyword list.
...
def get_google_ranking(keyword, domain, num_results=20):
    ...
    try:
        ...
        results = list(search(keyword, num_results=num_results, region="us",user_agent=headers))
